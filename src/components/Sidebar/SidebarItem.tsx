"use client";

import React from "react";
import Link from "next/link";
import SidebarDropdown from "@/components/Sidebar/SidebarDropdown";
import { usePathname } from "next/navigation";

interface SidebarItemProps {
  item: any;
  pageName: string;
  setPageName: (name: string) => void;
  currentPath: string;
}

const SidebarItem: React.FC<SidebarItemProps> = ({
  item,
  pageName,
  setPageName,
  currentPath,
}) => {
  const handleClick = () => {
    const updatedPageName =
      pageName !== item.label.toLowerCase() ? item.label.toLowerCase() : "";
    setPageName(updatedPageName);
  };

  // Determine if this item (or any of its children) is active.
  // For items with a valid route (not "#"), check if currentPath equals the route
  // or starts with route + "/" for sub-pages.
  const isActive = (item: any): boolean => {
    if (item.route === "#") return false;
    return (
      currentPath === item.route || currentPath.startsWith(item.route + "/")
    );
  };

  const isItemActive = isActive(item);

  return (
    <li>
      <Link
        href={item.route}
        onClick={handleClick}
        className={`group relative flex items-center gap-2.5 rounded-sm px-4 py-2 text-sm font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
          isItemActive ? "bg-primary dark:bg-primary" : ""
        }`}
      >
        {item.icon}
        {item.label}
        {item.children && (
          <svg
            className={`absolute right-4 top-1/2 -translate-y-1/2 fill-current ${
              pageName === item.label.toLowerCase() ? "rotate-180" : ""
            }`}
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M4.41107 6.9107C4.73651 6.58527 5.26414 6.58527 5.58958 6.9107L10.0003 11.3214L14.4111 6.91071C14.7365 6.58527 15.2641 6.58527 15.5896 6.91071C15.915 7.23614 15.915 7.76378 15.5896 8.08922L10.5896 13.0892C10.2641 13.4147 9.73651 13.4147 9.41107 13.0892L4.41107 8.08922C4.08563 7.76378 4.08563 7.23614 4.41107 6.9107Z"
              fill=""
            />
          </svg>
        )}
      </Link>
      {item.children && (
        <div
          className={`transform overflow-hidden ${pageName !== item.label.toLowerCase() ? "hidden" : ""}`}
        >
          <SidebarDropdown item={item.children} currentPath={currentPath} />
        </div>
      )}
    </li>
  );
};

export default SidebarItem;
