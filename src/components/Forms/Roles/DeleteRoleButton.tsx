...
const DeleteRoleButton: React.FC<DeleteRoleButtonProps> = ({ role }) => {
  ...
  async function deleteRole() {
    try {
      setLoading(true);
      const response = await axios.delete(`/api/v1/roles/${role.id}`, {
        headers: {
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent className="">
        <AlertDialogHeader>
          <AlertDialogTitle>
            {" "}
            Etes vous sûr de vouloir supprimer ce role?
          </AlertDialogTitle>
          <AlertDialogDescription>
