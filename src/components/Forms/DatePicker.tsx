...
import "flatpickr/dist/flatpickr.css"; // Ensure flatpickr styles are applied
...
interface DatePickerProps {
  ...
  defaultDate?: Date; // optional default date prop
}
...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const { field } = useController({
    name,
    control,
    defaultValue: defaultDate || "", // use defaultDate if provided
  });

  useEffect(() => {
    if (datePickerRef.current) {
      // Initialize flatpickr with the default date if provided
  ...
  // Update the flatpickr instance if defaultDate changes
  useEffect(() => {
    if (fpInstance.current && defaultDate) {
      fpInstance.current.setDate(defaultDate);
    }
  }, [defaultDate]);
