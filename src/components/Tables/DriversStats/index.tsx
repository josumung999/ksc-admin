...
type DriversStatsTableProps = {
  data: any[];
};

const DriversStatsTable: React.FC<DriversStatsTableProps> = ({ data }) => {
  // For deployment
  return (
          ...
          <tbody>
            {data.map((item: any, key: number) => (
              <tr key={key} className="text-sm">
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  <p className="text-black dark:text-white">
                    <p className="font-medium text-black dark:text-white">
                      {`${item?.vehicles[0]?.name}, ${item?.vehicles[0]?.brand ?? ""} ${item?.vehicles[0]?.model}`}
                    </p>

                    <p className="text-black dark:text-white">
                      {" "}
                      {item?.vehicles[0]?.immatriculation}
                    </p>
                  </p>
                </td>
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  <p className="text-black dark:text-white">
                    {formatNumber(item?.vehicles[0]?.livraisons?.length)}
                  </p>
                </td>
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
...
export default DriversStatsTable;
